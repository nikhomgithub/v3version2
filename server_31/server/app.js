...
//=====================
app.use("/p31upload",express.static('p31upload'));

app.get("/p31test",(req,res)=>{
    return res.status(200).json({msg:"Pass :you access to p31 server"});
})

const shopRt = require('./routes/shopRt');
app.use("/p31shop",shopRt);

const userRt = require('./routes/userRt');
app.use("/p31user",userRt);

const stickerRt = require('./routes/stickerRt');
app.use("/p31sticker",stickerRt);

const basicDataRt = require('./routes/basicDataRt');
app.use("/p31basicdata",basicDataRt);
